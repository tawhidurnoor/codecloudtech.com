...
@section('title')
    <title>Edit Hardware Product | CodeCloud Technology CMS</title>
@endsection
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.hardware.update', $hardwareProduct->id) }}" method="post"
                            enctype="multipart/form-data">
                            @csrf
                            @method('PUT')

                            <input type="hidden" name="id" value="{{ $hardwareProduct->id }}">

                            <div class="mb-3">
                                <label>Name</label>
                                <input type="text" name="title" value="{{ $hardwareProduct->title }}"
                                    onkeyup="makeSlug(this.value)" class="form-control" required>
                            </div>

                            <div class="mb-3">
                                <label>Product Slug</label>
                                <input type="text" value="{{ $hardwareProduct->slug }}" name="slug" id="slug"
                            ...
                            <div class="mb-3">
                                <label>Price</label>
                                <input type="number" value="{{ $hardwareProduct->price }}" name="price"
                            ...
                            <div class="mb-3">
                                ...
                                <textarea id="editor" name="content" required>{!! $hardwareProduct->content !!}</textarea>
                            </div>
...
@section('scripts')
    ...
    {{-- <script src="{{ asset('assets_backend/libs/ckeditor/ckeditor.js') }}"></script> --}}
        ...
        function checkSlug(val) {
            if (val.length >= 1) {
                ...
                $.ajax({
                    ...
                    data: "slug=" + val + "&product_id=" + "{{ $hardwareProduct->id }}",
